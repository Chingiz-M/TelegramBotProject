    ...
    internal class MobileMethods : IMobileMethods
    {
        ...
        public async Task Mobile_SelectServiceAsync(ITelegramBotClient botClient, long chatid, TgBotHostedService.TypeConnect connect)
        {
            string startText = $"Пожалуйста, выбери тип подключения:\n\n" +
                $"🔹IPSec(IKEv2) — (предпочтителен для iOS до версии 17.7) классический и самый быстрый мобильный протокол c шифрованием трафика — не нагружает устройство, напрямую поддерживается iOS, требует установки специального клиента на Android.\n\n" +
