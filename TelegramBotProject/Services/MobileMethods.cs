...
namespace TelegramBotProject.Services
{
    internal class MobileMethods : IMobileMethods
    {
        ...
        private string StartText = $"Вы выбрали опцию для мобильного устройства (iOS и Android)📱.";
        #endregion
        ...
        public async Task<string> Mobile_BotSocksKeyBalanserAsync()
        {
            ...
            return TgBotHostedService.SOCKS_SERVERS_LIST[1];
        }
        ...
        public async Task Mobile_BotStartAsync(ITelegramBotClient botClient, long chatid)
        {
            ...
            else // пользователь нажал старт и он есть в бд
            {
                string startText = $"Привет тебе, Путник!👋 Рады видеть тебя снова!\n\n" +
                    $"Ты использовал свой пробный период для мобильных устройств. Мы надеемся, что не разочаровали. 🙌\n" +
                    $"Чтобы продолжить пользоваться сервисом, необходимо приобрести подписку.\n\n" +
                    $"Стоимость подписки после пробного периода составит от 83 ₽/ мес.";
        ...
        public async Task Mobile_SelectServiceAsync(ITelegramBotClient botClient, long chatid, TgBotHostedService.TypeConnect connect)
        {
            string startText = $"Пожалуйста, выбери тип подключения:\n\n" +
                $"🔹IPSec(IKEv2) — (предпочтителен для iOS) классический и самый быстрый мобильный протокол c шифрованием трафика — не нагружает устройство, напрямую поддерживается iOS, требует установки специального клиента на Android.\n\n" +
                $"🔸Shadowsocks(Outline) — (предпочтителен для Android) современный, наиболее стабильный и надёжный протокол, основанный на маскирующем прокси SOCKS5 — минимально нагружает устройство, требует установки специального клиента с простейшей настройкой.\n\n";
        ...
        public async Task Mobile_BeginFreePeriodAsync(ITelegramBotClient botClient, Telegram.Bot.Types.CallbackQuery button, string typeconnect)
        {
            using (TgVpnbotContext db = new TgVpnbotContext())
            {
                ...
                else // если новый пользователь
                {
                    ...
                    await botClient.SendTextMessageAsync(chatID, $"Бесплатный период для мобильного устройства начался! ✅\n\n" +
